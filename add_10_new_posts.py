...
def main():
    ...
    print(f"現在の既存記事数: {len(existing_ids)} 件")
    print("追加で新しいギャル・美少女記事10件を取得・生成します...")
    
    items = fetch_new_fanza_items(existing_ids)
    print(f"新規取得できた作品数: {len(items)}")
    ...
    for idx, item in enumerate(items):
        ...
        print(f"追加成功 [{created_count+1}/{len(items)}]: {cid} - {post_data['title'][:30]}")
        created_count += 1
        
    print(f"追加生成が完了しました。現在の総記事数: {len(get_existing_ids())} 件")
